...
- The management of services, modules and instantiations in Typir-Langium is reworked:
  ...
  - Renamed utility function `createDefaultTypirLangiumServices` to `createDefaultTypirLangiumServicesModule` in `typir-langium.ts`.
- Renamed utility function `createDefaultTypirServiceModule` to `createDefaultTypirServicesModule` in `typir.ts`.
- Renamed the test utility `assertType` to `assertTypirType` in order to prevent accidental name collisions with `assertType` from `vitest`.
