...
export interface MultiplicityTypeDetails<Specifics extends TypirSpecifics> extends TypeDetails<Specifics> {
 ...
 */
export class MultiplicityKind<Specifics extends TypirSpecifics> implements Kind {
    readonly $name: string;
    readonly services: TypirServices<Specifics>;
    readonly options: Readonly<MultiplicityKindOptions>;

    constructor(services: TypirServices<Specifics>, options?: Partial<MultiplicityKindOptions>) {
    ...
    getMultiplicityType(typeDetails: MultiplicityTypeDetails<Specifics>): MultiplicityType | undefined {
        const key = this.calculateIdentifier(typeDetails);
        return this.services.infrastructure.Graph.getType(key) as MultiplicityType;
    }

    createMultiplicityType(typeDetails: MultiplicityTypeDetails<Specifics>): MultiplicityType {
        // check input
        ...
        const typeWithMultiplicity = new MultiplicityType(this as unknown as MultiplicityKind<TypirSpecifics>, this.calculateIdentifier(typeDetails), typeDetails);
    ...
    protected registerInferenceRules(_typeDetails: MultiplicityTypeDetails<Specifics>, _typeWithMultiplicity: MultiplicityType): void {
        // TODO
    }

    calculateIdentifier(typeDetails: MultiplicityTypeDetails<Specifics>): string {
...
export function isMultiplicityKind<Specifics extends TypirSpecifics>(kind: unknown): kind is MultiplicityKind<Specifics> {
