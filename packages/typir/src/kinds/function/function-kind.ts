...
export interface FunctionKindOptions<LanguageType> {
    ...
    typeToInferForCallsOfFunctionsWithoutOutput:
        | "THROW_ERROR"
        | TypeSelector<Type, LanguageType>;
...
export interface FunctionTypeDetails<LanguageType>
...
export interface CreateFunctionTypeDetails<LanguageType>
    extends FunctionTypeDetails<LanguageType> {
    inferenceRulesForDeclaration: Array<
        InferCurrentTypeRule<FunctionType, LanguageType>
    >;
    inferenceRulesForCalls: Array<
        InferFunctionCall<LanguageType, LanguageType>
    >;
}
...
> extends InferCurrentTypeRule<FunctionType, LanguageType, T> {
    /**
...
export interface FunctionFactoryService<LanguageType> {
    ...
    ): FunctionConfigurationChain<LanguageType>;
    get(
        typeDetails: FunctionTypeDetails<LanguageType>,
    ): TypeReference<FunctionType, LanguageType>;
    ...
    createUniqueFunctionValidation(
...
export interface FunctionConfigurationChain<LanguageType> {
    ...
    inferenceRuleForDeclaration<T extends LanguageType>(
        rule: InferCurrentTypeRule<FunctionType, LanguageType, T>,
    ): FunctionConfigurationChain<LanguageType>;
    /** for function calls => returns the return type of the function */
    inferenceRuleForCalls<T extends LanguageType>(
        rule: InferFunctionCall<LanguageType, T>,
    ): FunctionConfigurationChain<LanguageType>;
 ...
 */
export class FunctionKind<LanguageType>
    implements Kind, FunctionFactoryService<LanguageType>
{
    ...
    constructor(
        ...
        options?: Partial<FunctionKindOptions<LanguageType>>,
    ) {
    ...
    protected collectOptions(
        options?: Partial<FunctionKindOptions<LanguageType>>,
    ): FunctionKindOptions<LanguageType> {
        return {
            // the default values:
    ...
    get(
        ...
        return new TypeReference<FunctionType, LanguageType>(
    ...
    create(
        ...
        return new FunctionConfigurationChainImpl(
            this.services,
            this,
            typeDetails,
        );
    ...
    getOutputTypeForFunctionCalls(
        ...
        return (
            functionType.getOutput("RETURN_UNDEFINED")?.type ?? // by default, use the return type of the function ...
    ...
    calculateIdentifier(
        ...
        const functionName = this.hasFunctionName(typeDetails.functionName)
            ? typeDetails.functionName
            : "";
        // inputs: type identifiers in defined order
        const inputsString = typeDetails.inputParameters
            .map((input) =>
                this.services.infrastructure.TypeResolver.resolve(
                    input.type,
                ).getIdentifier(),
        ...
        const outputString = typeDetails.outputParameter
            ? this.services.infrastructure.TypeResolver.resolve(
...
export function isFunctionKind<LanguageType>(
    kind: unknown,
): kind is FunctionKind<LanguageType> {
    return isKind(kind) && kind.$name === FunctionKindName;
}

class FunctionConfigurationChainImpl<LanguageType>
    implements FunctionConfigurationChain<LanguageType>
{
    ...
    constructor(
        services: TypirServices<LanguageType>,
        kind: FunctionKind<LanguageType>,
        typeDetails: FunctionTypeDetails<LanguageType>,
    ) {
        this.services = services;
        this.kind = kind;
    ...
    inferenceRuleForDeclaration<T extends LanguageType>(
        ...
        this.currentFunctionDetails.inferenceRulesForDeclaration.push(
            rule as unknown as InferCurrentTypeRule<FunctionType, LanguageType>,
        );
        return this;
    }
    ...
    inferenceRuleForCalls<T extends LanguageType>(
        ...
        this.currentFunctionDetails.inferenceRulesForCalls.push(
            rule as unknown as InferFunctionCall<LanguageType>,
        );
        return this;
    }

    finish(): TypeInitializer<FunctionType, LanguageType> {
        return new FunctionTypeInitializer<LanguageType>(
            this.services,
            this.kind,
            this.currentFunctionDetails,
        );
