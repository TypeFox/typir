        ...
        RuleCollectorListener<SingleFunctionDetails<LanguageType>>
{
    ...
    onAddedRule(
        _rule: SingleFunctionDetails<LanguageType, LanguageType>,
    ...
    onRemovedRule(
        ...
        if (diffOptions.languageKey === undefined) {
            ...
            ).filter((key) => this.noFunctionCallRulesForThisLanguageKey(key));
            this.services.validation.Collector.removeValidationRule(this, {
                ...diffOptions,
    ...
    protected noFunctionCallRulesForThisLanguageKey(
    ...
    validation(
        ...
        const alreadyExecutedRules: Set<InferFunctionCall<LanguageType>> =
            new Set();
        // for each (overloaded) function
        for (const [
            overloadedName,
            overloadedFunctions,
        ] of this.functions.getAllOverloads()) {
            // this grouping is not required here (but for other use cases) and does not hurt here
            const resultOverloaded: Array<ValidationProblem<LanguageType>> = [];
            // for each language key
            for (const key of keysToApply) {
                for (const singleFunction of overloadedFunctions.details.getRulesByLanguageKey(
                    key,
                )) {
                    if (
                        alreadyExecutedRules.has(
                            singleFunction.inferenceRuleForCalls,
                        )
                    ) {
                        // TODO funktioniert das überhaupt, sprich: wird immer ein neues Objekt erstellt oder das aus der Konfiguration durchgereicht? zumindestens für Operatoren
                        // don't execute rules multiple times, if they are associated with multiple keys (with overlapping sub-keys)
                    } else {
                        const exactMatch = this.executeSingleRule(
                            singleFunction,
                            languageNode,
                            resultOverloaded,
                        );
                        if (exactMatch) {
                            ...
                            for (const specificValidation of toArray(
                                singleFunction.inferenceRuleForCalls.validation,
                            )) {
                                specificValidation.call(
                                    specificValidation,
                                    languageNode,
                                    singleFunction.functionType,
                                    accept,
                                    this.services,
                                );
                            }
     ...
     */
    protected executeSingleRule(
        singleFunction: SingleFunctionDetails<LanguageType>,
        languageNode: LanguageType,
        resultOverloaded: Array<ValidationProblem<LanguageType>>,
    ): boolean {
        const inferenceRule = singleFunction.inferenceRuleForCalls;
        const functionType = singleFunction.functionType;
        if (
            inferenceRule.filter !== undefined &&
            inferenceRule.filter(languageNode) === false
        ) {
            return false; // rule does not match at all => no constraints apply here => no error to show here
        }
        if (
            inferenceRule.matching !== undefined &&
            inferenceRule.matching(languageNode, functionType) === false
        ) {
        ...
        if (parameterLength.length >= 1) {
            ...
            const inferredParameterTypes = inputArguments.map((p) =>
                this.services.Inference.inferType(p),
            );
        ...
        if (currentProblems.length >= 1) {
            ...
            if (
                this.validateArgumentsOfFunctionCalls(
                    inferenceRule,
                    languageNode,
                )
            ) {
    ...
    protected validateArgumentsOfFunctionCalls<LanguageType>(
        rule: InferFunctionCall<LanguageType>,
        languageNode: LanguageType,
    ): boolean {
