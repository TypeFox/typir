 ...
 */
export class FunctionTypeInitializer<LanguageType>
    extends TypeInitializer<FunctionType, LanguageType>
    implements TypeStateListener
{
    ...
    constructor(
        ...
        typeDetails: CreateFunctionTypeDetails<LanguageType>,
    ) {
        super(services);
        ...
        // check the input
        if (
            typeDetails.outputParameter === undefined &&
            typeDetails.inferenceRulesForCalls.length >= 1
        ) {
            // no output parameter => no inference rule for calling this function
            throw new Error(
                `A function '${functionName}' without output parameter cannot have an inferred type, when this function is called!`,
            );
        }
        kind.enforceFunctionName(
            functionName,
            kind.options.enforceFunctionName,
        );

        // create the new Function type
        this.initialFunctionType = new FunctionType(
            kind as FunctionKind<unknown>,
            typeDetails as FunctionTypeDetails<unknown>,
        );

        this.inferenceRules = this.createInferenceRules(
            this.initialFunctionType,
        );
    ...
    onSwitchedToIdentifiable(functionType: Type): void {
        ...
        this.functions.addFunction(
            readyFunctionType,
            this.typeDetails.inferenceRulesForCalls,
        );
    }
    ...
    protected registerRules(
        ...
        for (const rule of this.inferenceRules.inferenceForCall) {
            const overloaded =
                this.functions.getOrCreateOverloads(functionName);
            overloaded.inferenceRule.addInferenceRule(
                rule.rule,
                optionsBoundToType(rule.options, functionType),
            );
        }
    ...
    protected deregisterRules(
        functionName: string,
        functionType: FunctionType | undefined,
    ): void {
        for (const rule of this.inferenceRules.inferenceForCall) {
            const overloaded = this.functions.getOverloads(functionName);
            overloaded?.inferenceRule.removeInferenceRule(
                rule.rule,
                optionsBoundToType(rule.options, functionType),
            );
        }
    ...
    protected createInferenceRules(
        functionType: FunctionType,
    ): FunctionInferenceRules<LanguageType> {
        ...
        for (const rule of this.typeDetails.inferenceRulesForDeclaration) {
            result.inferenceForDeclaration.push(
                bindInferCurrentTypeRule(rule, functionType),
            );
        }
    ...
    protected createFunctionCallInferenceRule(
        rule: InferFunctionCall<LanguageType>,
        functionType: FunctionType,
    ): TypeInferenceRule<LanguageType> {
        return new FunctionCallInferenceRule<LanguageType>(
            this.typeDetails,
            rule,
            functionType,
            this.functions,
