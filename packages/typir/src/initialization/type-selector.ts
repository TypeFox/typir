    ...
    resolve<T extends Type>(selector: TypeSelector<T, LanguageType>): T {
        if (isType(selector)) {
            ...
            return this.handleError(
                selector.getTypeFinal(),
                "This TypeInitializer don't provide a type.",
            );
        } else if (selector instanceof TypeReference) {
            return this.handleError(
                selector.getType(),
            ...
            if (isType(result)) {
                ...
                throw new Error(
                    `For '${this.services.Printer.printLanguageNode(selector, false)}' as TypeSelector, no type can be inferred.`,
                );
            }
        }
    }

    protected handleError<T extends Type>(
        result: T | undefined,
        errorMessage: string,
    ): T {
        if (result) {
