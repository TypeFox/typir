...
describe("Tiny Typir", () => {
    test("Set-up and test some expressions", async () => {
        ...
        // operators
        const inferenceRule: InferOperatorWithMultipleOperands<
            AstElement,
            BinaryExpression
        > = {
        ...
        expect(errorStack).includes(
            "The parameter 'right' at index 1 got a value with a wrong type.",
        );
        expect(errorStack).includes(
            "For property 'right', the types 'string' and 'number' do not match.",
        );
        ...
        expect(
            typir.validation.Collector.validate(assignNumberToString),
        ).toHaveLength(0);
