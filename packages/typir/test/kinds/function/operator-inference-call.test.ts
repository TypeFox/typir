...
import {
    isPrimitiveType,
    PrimitiveType,
} from "../../../src/kinds/primitive/primitive-type.js";
import {
    BinaryExpression,
...
describe("Tests some special cases for (overloaded) operator calls", () => {
    describe("Overloaded operators, one signature has no arguments at all (this is tests explicitly cover a found bug)", () => {
        ...
        beforeAll(() => {
            ...
            stringType = typir.factory.Primitives.create({
                ...
                .inferenceRule({
                    filter: (node) => node instanceof StringLiteral,
                })
                .finish();
    ...
    test("Overloaded operator has a validation for the inference rule of one signature", () => {
        ...
        typir.factory.Operators.createBinary({
            ...
            .inferenceRule({
                ...InferenceRuleBinaryExpression,
                validation: (node, name, opType, accept) =>
                    accept({
                        languageNode: node,
                        severity: "error",
                        message: `Called '${name}' with '${opType.getOutput()!.type.getName()}'.`,
                    }),
            })
            .finish();
        ...
        expectValidationIssuesStrict(
            typir,
            new BinaryExpression(integer123, "+", integer456),
            ["Called '+' with 'integer'."],
        );
        expectValidationIssuesStrict(
            typir,
...
function expectInferredType(
    typir: TypirServices<TestLanguageNode>,
    ...
    if (isType(result)) {
        expectToBeType(
            result,
            isPrimitiveType,
            (result) => result.getName() === expectedType,
        );
    } else {
        expect.fail(
...
function expectInferenceProblem(
    typir: TypirServices<TestLanguageNode>,
    left: TestExpressionNode,
    operator: "+",
    right: TestExpressionNode,
