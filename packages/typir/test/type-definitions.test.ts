...
import {
    FunctionKind,
    NO_PARAMETER_NAME,
} from "../src/kinds/function/function-kind.js";
...
describe("Tests for Typir", () => {
    test("Define some types", async () => {
        ...
        const typir = createTypirServices({
            ...
            factory: {
                Classes: (services) =>
                    new ClassKind(services, {
                        typing: "Structural",
                        maximumNumberOfSuperClasses: 1,
                        subtypeFieldChecking: "SUB_TYPE",
                    }),
            },
        });

        // reuse predefined kinds
        const multiplicityKind = new MultiplicityKind(typir, {
            symbolForUnlimited: "*",
        });
        const listKind = new FixedParameterKind(
            typir,
            "List",
            { parameterSubtypeCheckingStrategy: "EQUAL_TYPE" },
            "entry",
        );
        const mapKind = new FixedParameterKind(
            typir,
            "Map",
            { parameterSubtypeCheckingStrategy: "EQUAL_TYPE" },
            "key",
        ...
        const typeString = typir.factory.Primitives.create({
            ...
            .inferenceRule({
                filter: (languageNode) => typeof languageNode === "string",
            })
            .finish(); // combine type definition with a dedicated inference rule for it
        const typeBoolean = typir.factory.Primitives.create({
            primitiveName: "Boolean",
        }).finish();

        // create class type Person with 1 firstName and 1..2 lastNames and an age properties
        const typeOneOrTwoStrings = multiplicityKind.createMultiplicityType({
            constrainedType: typeString,
            lowerBound: 1,
            upperBound: 2,
        });
        ...
        const typeStudent = typir.factory.Classes.create({
            ...
            fields: [{ name: "studentNumber", type: typeInt }],
            methods: [],
        }).finish();

        // create some more types
        const typeListInt = listKind.createFixedParameterType({
            parameterTypes: typeInt,
        });
        const typeListString = listKind.createFixedParameterType({
            parameterTypes: typeString,
        });
        ...
        const opAdd = typir.factory.Operators.createBinary({
            name: "+",
            signature: { left: typeInt, right: typeInt, return: typeInt },
        }).finish();
        const opMinus = typir.factory.Operators.createBinary({
            name: "-",
            signature: { left: typeInt, right: typeInt, return: typeInt },
        }).finish();
        const opLess = typir.factory.Operators.createBinary({
            name: "<",
            signature: { left: typeInt, right: typeInt, return: typeBoolean },
        }).finish();
        const opEqualInt = typir.factory.Operators.createBinary({
            name: "==",
            signature: { left: typeInt, right: typeInt, return: typeBoolean },
        })
            .inferenceRule({
                filter: (languageNode): languageNode is string =>
                    typeof languageNode === "string",
                matching: (languageNode) => languageNode.includes("=="),
                operands: (languageNode) => [],
            })
            .finish();
        // binary operators on Booleans
        const opEqualBool = typir.factory.Operators.createBinary({
            name: "==",
            signature: {
                left: typeBoolean,
                right: typeBoolean,
        ...
        const opAnd = typir.factory.Operators.createBinary({
            name: "&&",
            signature: {
                left: typeBoolean,
                right: typeBoolean,
        ...
        // unary operators
        const opNotBool = typir.factory.Operators.createUnary({
            name: "!",
            ...
            .finish();
        // ternary operator
        const opTernaryIf = typir.factory.Operators.createTernary({
            name: "if",
            signature: {
                first: typeBoolean,
                second: typeInt,
        ...
        expect(typir.Assignability.isAssignable(typeInt, typeString)).toBe(
            true,
        );
        expect(typir.Assignability.isAssignable(typeString, typeInt)).not.toBe(
            true,
        );
        // List, Map
        // expect(typir.assignability.isAssignable(typeListInt, typeMapStringPerson)).not.toBe(true);
        expect(
            typir.Assignability.isAssignable(typeListInt, typeListString),
        ).not.toBe(true);
        expect(typir.Assignability.isAssignable(typeListInt, typeListInt)).toBe(
            true,
        );
        // classes
