...
export type LangiumTypeInferenceRules<T extends LangiumAstTypes> = {
    [K in keyof T]?: T[K] extends AstNode
        ?
              | TypeInferenceRule<AstNode, T[K]>
              | Array<TypeInferenceRule<AstNode, T[K]>>
        : never;
} & {
    AstNode?:
        | TypeInferenceRule<AstNode, AstNode>
        | Array<TypeInferenceRule<AstNode, AstNode>>;
};

export interface LangiumTypeInferenceCollector<AstTypes extends LangiumAstTypes>
    extends TypeInferenceCollector<AstNode> {
    addInferenceRulesForAstNodes(
        rules: LangiumTypeInferenceRules<AstTypes>,
    ): void;
}
